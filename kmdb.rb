...
#update the tables accordingly in db > migrate

#then run rails db:migrate to make the changes 
...
new_movie1["studio_id"] = Studio.find_by({ "name" => "Warner Bros." }).id
new_movie1.save
...
new_movie2["studio_id"] = Studio.find_by({ "name" => "Warner Bros." }).id
new_movie2.save
...
new_movie3["studio_id"] = Studio.find_by({ "name" => "Warner Bros." }).id
new_movie3.save
...
new_role1 = Role.new
new_role1["movie_id"] = Movie.find_by({ "title" => "Batman Begins" }).id
new_role1["actor_id"] = Actor.find_by({ "name" => "Christian Bale" }).id
...
#could have used formatting from chatgpt to look exactly like the output (organized columns)

batman_movies = Movie.where({ "title" => ["Batman Begins", "The Dark Knight", "The Dark Knight Rises"] })
